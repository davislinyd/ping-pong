...
本方法把主估計值與診斷指標分開。`Download` 與 `Upload` 主卡顯示的是 **stable mean**，也就是經過啟動段裁切與 IQR 離群值過濾後的穩定吞吐平均。另一方面，`P10`、`P50`、`P75/Q3`、`P90`、`Raw CV`、`Stable CV`、`IQR kept`、`Jitter` 與 `HTTP Loss` 用來描述波動、低速體感、離群率與可靠性。
...
P75 與 P90 描述高段能力。P75 同時也是 Q3，也就是 IQR 計算中的第三四分位數。它們可協助判斷連線是否偶爾能衝到高 throughput，但不能單獨代表穩定品質。

### 為什麼主卡 detail 顯示 P75/Q3，但不顯示 P25/Q1

P25/Q1 有統計價值，因為 IQR fence 需要 Q1 與 Q3 才能計算；但它在 summary UI 中不是最直覺的診斷指標。P10 已經負責低速體感，P50 負責典型值，P75/Q3 與 P90 負責上段能力。若再加入 P25/Q1，會增加卡片資訊密度，卻不一定改善一般使用者的判讀。

因此，Q1/P25 保留為統計原理與 Raw Data 解釋概念。若需要完整重算 IQR，應檢查 Raw Data 的 sample classification 或未來 advanced diagnostics，而不是依賴主卡 detail。
...
卡片 detail 也顯示 outlier rate：

```text
outlierRate = (sampleCount - filteredSampleCount) / sampleCount
```

若 `sampleCount = 0`，outlier rate 顯示為 0%。如果 kept 比例很低，代表資料中有大量離群點。這不一定表示測試錯誤，但表示結果更像診斷事件，而不是單純穩定能力測量。
...
| IQR kept | IQR 過濾後保留的樣本數與 startup trim 後樣本數比例，用來顯示 stable mean 依據多少資料。 |
